        ...
        <div class="right_box">
            ...
            <div class="table_main" style="width: 99%;padding-top:30px;">
                <el-table ref="multipleTable" align="left" :data="tableData" border element-loading-text="拼命加载中" tooltip-effect="dark" style="width: 100% ; border-radius:8px; box-shadow:1px 0px 4px 4px #f2f2f2;">
                    ...
                    <el-table-column prop="occupation" label="职业" align="center"> </el-table-column>
...
export default {
  ...
  methods: {
    ...
    drawPie() {
      var myChart = echarts.init(document.getElementById('main'))
      var colorlist = ['#019e97','#9d076d','#5cba2e','#009f69','#600b80',
                      '#392a87','#fcf97c','#8d0069','#fecc77',
                      '#0168b7','#1d2089','#611987','#920784',
                      '#e60412','#eb6100','#f39801','#fcc900',
                      '#fff100','#90c320','#009a44'];
      // myChart.clear();
      myChart.setOption(
        {
          tooltip: {
    ...
    getAttackList() {
      ...
      this.percentList = []
      axios
        .get(config.apiHost + '/count/countoccupation', {
          params: {
        ...
        .then(result => {
          if (result.data.status == 200) {
            ...
            result.data.body.forEach(item => {
              ...
              this.nameList.push(item.occupation)
              var ness = 0
              var per=0;
              if (sum == 0) {
                ness = '0.00%';
                ...
                ness = (item.personNum / sum * 100).toFixed(2) + '%'
                per=(item.personNum / sum * 100).toFixed(2)
              }
              ...
              let user = { value: item.personNum, name: item.occupation }
