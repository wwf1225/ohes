...
export default {
  data() {
    return {
      ...
      limit: 1,
      uploadUrl: config.apiHost + "/import/importReprtSystem", //导入接口
      importData: {
        token: config.token
      },
      fileName: "",
  ...
  methods: {
    ...
    getTableListData() {
      ...
      axios({
        ...
        params: {
          ...
          diseaseId: this.selectDatas.length > 0 ? this.form.diseaseId : "",
      ...
      if (this.isShowDis) {
        axios({
          ...
          if (result.data.status == 200) {
            ...
            this.defalutSelect = [];
            result.data.body.forEach(v => {
              if (v.children.length > 0) {
                v.children.forEach(v1 => {
                  if (v1.label == this.flagName) {
                    this.defalutSelect = [v.value, this.form.diseaseId];
                    return false;
                  }
                });
              } else {
                if (v.label == this.flagName) {
                  this.defalutSelect = [v.value, this.form.diseaseId];
                  return false;
                }
              }
        ...
        AMap.plugin("AMap.Geocoder", function() {
          ...
          geocoder.getAddress(lnglat, function(status, result) {
            if (status === "complete" && result.info === "OK") {
              ...
              self.form.township = result.regeocode.addressComponent.township;
              self.form.street = result.regeocode.addressComponent.street;
              self.fromShow = false;
              self.clearInput();
            }
          });
    ...
    clearInput() {
      this.$refs.locationSearchInput1.value = "";
    },
    showMore(e) {
      // more
