  ...
  <div class="rolebox">
    ...
    >
      <div class="divbox">
        ...
        <div class="footer">
          <el-button size="medium" @click="issue()" style="float:left">确认</el-button>
          <el-button size="medium" @click="closeissue()" style="float:right">取消</el-button>
        </div>
      </div>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    closepoint() {
      var arr = Object.keys(this.map);
      var azrr = Object.keys(this.marker);
      if (arr.length > 0 && azrr > 0) {
    ...
    showMap() {
      this.fromShow = true;
      // console.log("center", this.longitude, this.latitude);
      this.map.setCenter(new BMap.Point(this.longitude, this.latitude));
      // 地图view 宽高 800*600
      this.map.panBy(400, 300);
      //   this.getMap();
    },
    getLocal() {
      //获取地理位置
          ...
          myGeo.getLocation(
            ...
            result => {
              if (result) {
                ...
                self.oldlongitude = result.point.lng;
                self.oldlatitude = result.point.lat;
                self.getMap();
              }
            }
          );
    ...
    onSuccess(position) {
      ...
      this.oldlongitude = position.coords.longitude;
      //纬度
      this.latitude = position.coords.latitude;
      this.oldlatitude = position.coords.latitude;
      this.getMap();
      //使用百度地图API
      //创建地图实例
    ...
    getMap() {
      ...
      // this.latitude += 0.636864
      // this.longitude -= 0.778122
      var point = new BMap.Point(this.longitude, this.latitude);
