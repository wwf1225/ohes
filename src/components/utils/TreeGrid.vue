                ...
                <tr v-for="(item,index) in initItems" :key="item.id" v-show="show(item)" :class="{'child-tr':item.parent}">
                    <td v-for="(column,snum) in columns" :key="column.key" :style="tdStyle(column)" >
                        ...
                        <div v-if="column.type === 'action'">
                            <i-button :type="column.actions[0].type" size="small" @click="RowClick(item,$event,item.id)" >{{column.actions[0].text}}</i-button>
                             <i-button :type="column.actions[1].type" size="small" @click="TwoClick(item,$event,item.id)" >{{column.actions[1].text}}</i-button>
                        </div>
                        <label @click="toggle(index,item)" v-if="!column.type">
                            <span v-if='snum==iconRow()'>
                                <i v-html='item.spaceHtml'></i>
                                <i v-if="item.list&&item.list.length>0" class="ivu-icon" :class="{'el-icon-caret-right':!item.expanded,'el-icon-caret-top':item.expanded }"></i>
    ...
    methods: {
      ...
      iconRow() {
        ...
        RowClick(data, event,id) {
            ...
            this.$emit('on-row-click', result, event,id)
        },
        // 点击删除某一行事件
        TwoClick(data, event,id) {
            let result = this.makeData(data)
            this.$emit('on-two-click', result, event,id)
        },
        selectionClick(arr) {
        ...
        initData(items, level, parent) {
            ...
            items.forEach((item, index) => {
                ...
                if (item.list && item.expanded) {
                    this.initData(item.list, level + 1, item);
                }
            })
        },
        //  隐藏显示
        ...
        All(data) {
            ...
            data.forEach((item) => {
                ...
                if (item.list && item.list.length > 0) {
                    arr = arr.concat(this.All(item.list));
                }
            })
            return arr
        },
        ...
        renderCheck(data) {
            ...
            data.forEach((item) => {
                ...
                if (item.list && item.list.length > 0) {
                    arr = arr.concat(this.renderCheck(item.list));
                }
            })
            return arr
        },
